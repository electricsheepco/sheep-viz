...
class HardwareControls {
    constructor(config) {
        console.log('HardwareControls: Constructor starting...');
        try {
            this.config = {
                name: 'Visualizer',
                accentColor: '#00ff88',
            ...
            console.log('HardwareControls: Config merged');
            ...
            console.log('HardwareControls: State initialized');

            console.log('HardwareControls: Creating UI...');
            this._createHardwareBar();
            this._createToggleButton();
            this._createWatermark();
            this._setupEventListeners();
            this._applyAccentColor();
            console.log('HardwareControls: UI created');

            console.log('HardwareControls: Initializing MIDI...');
            this._initMIDI();
            console.log('HardwareControls: Ready!');
        } catch (err) {
    ...
    _initMIDI() {
        ...
        navigator.requestMIDIAccess({ sysex: false }).then(access => {
            ...
            console.log(`MIDI: ${this.midiInputs.length} device(s) found`);
            // Show first controller name (prefer "MiniLab" if found)
            ...
            this.midiInputs.forEach(input => {
                console.log(`MIDI: Connecting to ${input.name}...`);
                input.onmidimessage = (e) => {
                    console.log('MIDI message received from', input.name);
                    this._handleMIDI(e);
                };
            ...
            access.onstatechange = (e) => {
                if (e.port.type === 'input') {
                    if (e.port.state === 'connected') {
                        ...
                        console.log(`MIDI: ${e.port.name} connected`);
                    } else {
                        console.log(`MIDI: ${e.port.name} disconnected`);
                    }
                }
            };
        ...
        console.log('MIDI: Waiting for permission...');
    }
    ...
    _handleMIDI(e) {
        ...
        // Debug logging
        console.log(`MIDI: ch=${channel + 1} type=${msgType.toString(16)} data1=${data1} data2=${data2}`);
    ...
    _setupEventListeners() {
        console.log('HardwareControls: Setting up event listeners...');
        ...
        this.bar.querySelectorAll('.sknob').forEach(knob => {
            ...
            outer.addEventListener('mousedown', (e) => {
                console.log('Knob mousedown:', knob.dataset.param);
