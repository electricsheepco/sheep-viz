...
class EmbedAdapter {
  ...
  _listenForMessages() {
    window.addEventListener('message', (e) => {
      ...
      switch (msg.type) {
        ...
        case 'watermark':
          this._applyWatermarkSettings(msg);
          break;
      }
    });
  }
  ...
  /**
   * Apply watermark settings from Control Room.
   * Updates logo, visibility, size, and opacity.
   */
  _applyWatermarkSettings(config) {
    // Wait for hardware controls to be initialized
    if (!window.hardware) {
      setTimeout(() => this._applyWatermarkSettings(config), 100);
      return;
    }

    // Update logo via hardware-controls API
    if (config.logo) {
      window.hardware.setLogo(config.logo, 'Band Logo');
    }
